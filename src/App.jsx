...
function App() {
  ...
  // Check if any modal is open (About or Project Detail)
  const modalIsOpen = () =>
    !!document.querySelector('.project-detail-overlay.open, .modal-overlay.open');
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      if (isScrolling) return;
      if (modalIsOpen()) return; // block while modal open
  ...
  useEffect(() => {
    const handleTouchStart = (e) => {
      if (modalIsOpen()) {
        // ignore gesture while modal is open
  ...
  // ArrowUp/ArrowDown to navigate (looping)
  useEffect(() => {
    const handleKey = (e) => {
      if (isScrolling) return;
      if (modalIsOpen()) return; // block while modal open
